...
class FFNNPreTrainedModel(PreTrainedModel):
    config_class = FFNNConfig
    base_model_prefix = "ffnn"

    def _init_weights(self, module: nn.Module) -> None:
        """Initialize the weights."""
        if isinstance(module, nn.Linear):
            # Slightly different from the TF version which uses truncated normal
...
class FFNNEmbeddings(nn.Module):
    """
...
class FFNNModel(FFNNPreTrainedModel):
    def __init__(self, config: PretrainedConfig) -> None:
        super().__init__(config)
        self.config = config
        self.embeddings = FFNNEmbeddings(config)
...
class FFNNForSequenceClassification(FFNNPreTrainedModel):
    def __init__(self, config: PretrainedConfig) -> None:
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config

        self.ffnn = FFNNModel(config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        logits = torch.mean(self.classifier(x), dim=1)
